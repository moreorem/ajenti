...
def is_bound(el):
    ...
    if el.typeid.startswith('bind:'):
        return True
    for prop in list(el.properties.keys()):
...
class PropertyBinding (Binding):
    ...
    def __init__(self, obj, attribute, ui, property=None):
        ...
        if property is None:
            ...
            for prop in list(ui.property_definitions.values()):
                if prop.bindtypes:
    ...
    def __repr__(self):
        return '[%s.%s <-> %s.%s]' % (self.object, self.attribute, self.ui, self.property)
...
class ListAutoBinding (Binding):
    ...
    def unpopulate(self):
        for binder in list(self.binders.values()):
            binder.unpopulate()
