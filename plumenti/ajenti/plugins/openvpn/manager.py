...
class manager(object):
        ...
        for line in self.execute("status", "2"):
            ...
            if fields[0] == "TITLE":
                ...
                status["clients"].append(dict(
                    list(zip(("cn", "raddress", "vaddress", "brecv", "bsent", "connsince", "connsinceu"), fields[1:]))
                ))
    ...
    def messages(self):
        ...
        return [x.split(",") for x in self.execute("log", "all")]
    ...
    def _pgenresp(response):
        ...
        @return: Dictionary
        """
        return dict([x.split("=") for x in response.split(",")])
