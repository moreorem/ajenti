...
class PluginContext (object):
    ...
    def vacuum_instances(self):
        dead_ifaces = []
        for iface, lst in self.__instances.items():
            for ref in list(lst):
                if ref() is None:
                    lst.remove(ref)
            if len(lst) == 0:
    ...
    def get_instances(self, cls):
        return [_f for _f in [_() for _ in self.__instances.setdefault(cls, [])] if _f]
...
class PluginManager:
    ...
    def get_implementations(self, iface):
        return [x for x in self.__classes.setdefault(iface, []) if x not in self.blacklist]
