...
class Dialog (UIElement):
    ...
    def init(self):
        ...
        for button in self.buttons:
            if isinstance(button, str) and button in presets:
...
@p('text', default='Input value:', type=str)
@p('value', default='', bindtypes=[str, str], type=str)
@plugin
...
class BaseFileDialog (object):
    ...
    def refresh(self):
        ...
        if not os.path.exists(self.path):
            ...
            _dirs += sorted(os.listdir(str(self.path)))
...
@p('path', default='/', type=str)
@p('root', default='/', type=str)
@plugin
